...
/**
 * Created by joefan on 2016/12/21 0021.
 */
    ...
    public class NumberToChina {
        ...
        public static void main(String[] args) {
            System.out.println("\n--------将数字转换成中文金额的大写形式------------\n");
            String s = cleanZero1(splitNum1(roundString("4")));
            // 如果转换过后是一个空串，则不输出屏幕
            if(!"".equals(s)) {
         ...
         * @return 转换好的中文金额大写形式的字符串
         */
        public static String splitNum1(String s) {
            // 如果传入的是空串则继续返回空串
            if("".equals(s)) {
                return "";
            }
            ...
            // 截取并转换这个数的整数部分
            String intOnly = s.substring(0, index);
            String part1 = numFormat1(1, intOnly);
            // 截取并转换这个数的小数部分
            String smallOnly = s.substring(index + 1);
            String part2 = numFormat(2, smallOnly);
            // 把转换好了的整数部分和小数部分重新拼凑一个新的字符串
         ...
         * @return 改进后的字符串
         */
        public static String cleanZero1(String s) {
            // 如果传入的是空串则继续返回空串
            if("".equals(s)) {
                return "";
            }
         ...
         * @return 转换好的带单位的中文金额大写形式
         */
        private static String numFormat1(int flag, String s) {
            int sLength = s.length();
            // 货币大写形式
            String bigLetter[] = {"零", "一", "二", "三", "四", "五", "六", "七", "八", "九"};
            // 货币单位
            String unit[] = {"圆", "十", "百", "千", "万",
                    // 拾万位到仟万位
                    "十", "百", "千",
                    // 亿位到万亿位
                    "亿", "十", "百", "千", "万"};
